...
# Stage 2: Python Requirements Installation
...
# Stage 3: CMake Build with llama-cpp-python
FROM requirements-stage AS cmake-stage

RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential g++ cmake git pkg-config ninja-build \
    libopenblas-dev libomp-dev libssl-dev libgomp1 libbz2-dev liblzma-dev ca-certificates cuda-drivers \
    && rm -rf /var/lib/apt/lists/*

ENV CC=gcc CXX=g++

RUN CMAKE_ARGS="-DGGML_CUDA=on" FORCE_CMAKE=1 python3 -m pip install --no-cache-dir --force-reinstall --verbose llama-cpp-python
